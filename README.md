...
An AHU or HRV serving both sides has **four connections**, two on each
face, drawn on the casing:

| On the plan | Airstream | EN 16798-3 | Colour | Runs |
| --- | --- | --- | --- | --- |
| **IN** | Supply air | SUP | blue | unit → rooms |
| **IN** | Extract air | ETA | orange | rooms → unit |
| **EX** | Fresh (outdoor) air | ODA | green | outside → unit |
| **EX** | Exhaust air | EHA | brown | unit → outside |

Two connections on each face, each airstream running straight through the
box: the **internal** face (labelled **IN** on the plan) carries supply and
